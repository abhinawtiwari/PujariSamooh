...
class Peer:
    ...
    def broadcastIP(self):
        ...
        message = f'HOST {self.host} PORT {self.port} ACTION WATER '.encode('utf-8')
        while True:
    ...
    def sendData(self, data, command):
        ...
        if command == 'ALERT':
            ...
            for peer in self.filter_peers(interest='Water'):
                try:
                    ...
                    s.connect(peer_tup)
                    print('Maa chud gayi')
                    msg = f' WATER ALERT from {self.host}, WATER {data}'
                    s.send(msg.encode())
                    sent = True
                    s.close()
                except Exception:
